...
const RegisterPage = (props) => {
  ...
  const registerLogic = async (e) => {
    ...
    attributeList.push(setCognitoUserAttribute("email", data.email));
    userPool.signUp(
      data.email,
      data.password,
      attributeList,
      ...
      async (err, result) => {
        if (err) {
          ...
          const res = await axios
            .post(`${config.api.invokeurl}/users`, data)
            .then((r) => {
              const msg = "succufully created user";
              setMessage(msg);
              setMessageType("ok");
              setLoading(false);
